...
def main():
    ...
    for index, (language, count) in enumerate(rows):
        ...
        body.append(
            f'<rect x="24" y="{y + 7}" width="308" height="5" rx="2.5" fill="var(--track)"/>'
        )
        body.append(
            f'<rect x="24" y="{y + 7}" width="{308 * count / total:.2f}" height="5" rx="2.5" fill="{COLORS.get(language, "#7886a0")}"/>'
        )
    if not rows:
    ...
    body.append(
        text(
            24,
            244,
            "By bytes · public originals · excludes this profile",
            10,
            "var(--muted)",
        )
    )
    language_svg = card(
        ...
        "Most Used Languages",
        "\n".join(body),
        "; ".join(summary) or "No language data",
    )
    ...
    body.append(
        text(24, 244, f"Public data only · updated {updated} UTC", 11, "var(--muted)")
    )
    stats_svg = card(
        470,
        "Specode's GitHub Stats",
        "\n".join(body),
